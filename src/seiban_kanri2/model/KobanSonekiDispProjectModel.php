...
class KobanSonekiDispProjectModel
{
    ...
    */
    public function getKobanList($no_project)
    {
        ...
        $result2 = $this->getKobanTyouseiList($no_project);
        
        $hasKeys = function ($table, $no_cyu, $no_ko) {
            foreach ($table as $list) {
                if (($list['no_cyu'] == $no_cyu) && ($list['no_ko'] == $no_ko)) {
                    return $list;
                }
            }
            return [];
    ...
    *   項番調整値データ取得
    *
    *   @param string $no_project
    *   @return array
    */
    protected function getKobanTyouseiList($no_project)
    {
        $sql = "
            SELECT B.*
