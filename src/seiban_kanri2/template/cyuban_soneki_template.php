...
<?php require_once('../_template/header_date.php'); ?>
<?php require_once('../_template/header_strtotime.php'); ?>
...
<option value=""></option>
<option value="all" <?php if ($cd_bumon == 'all') {
    echo 'selected';
                    } ?>>全て</option>

<?php foreach ((array)$bumon_list as $list) : ?>
<option value="<?= $list['cd_bumon']; ?>" <?php if ($list['cd_bumon'] == $cd_bumon) {
    echo'selected';
               } ?>><?= $list['nm_bumon']; ?></option>
<?php endforeach; ?>
...
<?php foreach ((array)$nendo_list as $list) : ?>
<option value="<?= $list['kb_nendo']; ?>" <?php if ($list['kb_nendo'] == $kb_nendo) {
    echo 'selected';
               } ?>><?= $list['nm_nendo']; ?></option>
<?php endforeach; ?>
...
<td>
<?php if (!$isPastFiscalYear) : ?>
<input type="checkbox" name="chk_nendo_all" id="_chk_nendo_all" value="<?= $chk_nendo_all; ?>" onClick="sel_check(this)" <?php if ($chk_nendo_all) {
    echo 'checked';
                                                                       } ?>>
<label for="_chk_nendo_all" title="チェックで当期以降データを表示条件とします">当期以降</label>
<?php endif; ?>

<input type="checkbox" name="chk_kansei" id="_chk_kansei" value="<?= $chk_kansei; ?>" onClick="sel_check(this)" <?php if ($chk_kansei) {
    echo 'checked';
                                                                 } ?>>
<label for="_chk_kansei" title="チェックで売上済データを非表示条件とします">仕掛</label>

<input type="checkbox" name="fg_cyuban" id="_fg_cyuban" value="<?= $fg_cyuban; ?>" onClick="sel_check(this)" <?php if ($fg_cyuban) {
    echo 'checked';
                                                               } ?>>
<label for="_fg_cyuban" title="チェックで選択部門の項番だけで集計したデータを一覧表の表示条件とします">課内</label>

<input type="checkbox" name="chk_job" id="_chk_job" value="<?= $chk_job; ?>" onClick="check_tanto(this)" <?php if ($chk_job) {
    echo 'checked';
                                                           } ?>>
...
<option value="<?= $list['cd_tanto']; ?>" <?php if ($list['cd_tanto'] == $narrow_tanto) {
    echo 'selected';
...
<?php require_once('../_template/environment_button.php'); ?>

</form>

<td><a class="ui-state-default" href="/help/cyuban_soneki_disp.pdf" target="_blank" title="操作マニュアルを表示します">？</a></td>
</tr>
</table>
...
<?php foreach ((array)$kisyu_list as $list) : ?>
<option value="<?= $list['cd_kisyu']; ?>" <?php if ($list['cd_kisyu'] == $cd_kisyu) {
    echo'selected';
               } ?>><?= $list['nm_kisyu']; ?></option>
<?php endforeach; ?>
...
<?php foreach ((array)$bunya_eigyo_list as $list) : ?>
<option value="<?= $list['no_bunya']; ?>" <?php if ($list['no_bunya'] === $no_bunya_eigyo) {
    echo'selected';
               } ?>><?= $list['nm_bunya']; ?></option>
<?php endforeach; ?>
...
<?php foreach ((array)$bunya_seizo_list as $list) : ?>
<option value="<?= $list['no_bunya']; ?>" <?php if ($list['no_bunya'] == $no_bunya_seizo) {
    echo'selected';
               } ?>><?= $list['nm_bunya']; ?></option>
<?php endforeach; ?>
...
<tr>
<th>予測&nbsp;<a href="/help/cyuban_soneki_disp2.pdf" target="_blank" title="予測値の計算式を表示します">?</a></th>
