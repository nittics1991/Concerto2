...
<?php require_once('../_template/header_sprintf.php'); ?>
...
<?php if ($fg_crt == '1') : ?>
    ...
    <td><a class="ui-state-default" href="/help/cyunyu_inf_disp.pdf" target="_blank" title="操作マニュアルを表示します">？</a></td>
<?php else : ?>
    <td><a class="ui-state-default" href="koban_soneki_disp.php?no_cyu=<?= $no_cyu; ?>" target="_top" title="製番別詳細情報画面に戻ります">戻る</a></td>
    ...
    <?php if ((($kengen_sm >= '2') && ($kengen_sm <= '3')) || ($kengen_db == '1')) : ?>
        <td>
        <?php if (!$fg_lock) : ?>
...
<?php if ($fg_crt == '0') : ?>
    ...
    <?php if (empty($no_seq)) : ?>
        ...
        <?php foreach ($kb_nendo_list as $list) : ?>
        ...
        <?php $i = 0; ?>
            <?php foreach ((array)$genka_yoso_list as $key => $val) : ?>
                <input type="radio" name="cd_genka_yoso" id="r<?= $i; ?>" value="<?= $key; ?>" onClick="sel_youso(this)" <?php if ($key == $cd_genka_yoso) {
                    echo 'checked';
                                                              } ?> required>
        ...
        <td class="td11">
        <select name="cd_bumon" id="cd_bumon" onChange="sel_bumon(this)" <?php if ($cd_genka_yoso == 'A') {
            echo 'disabled';
                                                                         } ?> title="注入先原価部門を選択します">
        ...
        <select name="cd_tanto" id="cd_tanto" <?php if ($cd_genka_yoso == 'A') {
            echo 'disabled';
                                              } ?> title="注入先担当を選択します">
        <option value="XXXXXXXX">注入先指定</option>
        
        <?php foreach ((array)$tanto_list as $list) : ?>
            <option value="<?= $list['cd_tanto']; ?>" <?php if ($list['cd_tanto'] == $cd_tanto) {
                echo 'selected';
    ...
    <input type="text" name="nm_tanto" id="nm_tanto" value="<?= $nm_tanto; ?>" <?php if (($cd_genka_yoso != 'A') || (!empty($no_seq))) {
        echo 'class="input-text-readonly text11" readonly';
                                                            } else {
                                                                echo 'class="text11"';
                                                            } ?> title="記号は全角文字" pattern="^[^｡-ﾟ\x00-\x1f\x7f\x20-\x2f\x3a-\x40\x5b-\x60\x7b-\x7e]*$">
    </td>
    
    <tr>
    <th class="th11">商品名称</th>
    <td class="td11">
    <input type="text" name="nm_syohin" id="nm_syohin" value="<?= $nm_syohin3; ?>" <?php if (!empty($no_seq)) {
        echo 'class="input-text-readonly td11" readonly';
                                                              } else {
                                                                  echo 'class="td11"';
                                                              } ?> title="記号は全角文字" pattern="^[^｡-ﾟ\x00-\x1f\x7f\x20-\x2f\x3a-\x40\x5b-\x60\x7b-\x7e]*$">
    </td>
